...
assert.match(html, /mark\.keyword-highlight\s*\{[^}]*background:\s*transparent;[^}]*font-weight:\s*800;[^}]*text-decoration-line:\s*underline;/s);
assert.match(html, /body\.dark-mode mark\.keyword-highlight\s*\{[^}]*color:\s*#ffd166;/s);
assert.match(html, /Trích Từ Khóa Tô Sáng/);
assert.match(html, /name="application-version" content="1\.4\.4"/);
